...
class OrderBookRepository extends ServiceEntityRepository
{
    ...
    public function findProcessedOrders($userId)
    {
        ...
        $sql = '
            select books.image,books.name,books.author,books.published_date from books
            inner join order_book
            where books.id=order_book.book_id_id
            AND order_book.user_email_id= :id AND order_book.processing=true;
        ';
    ...
    public function findunProcessedOrders($userId)
    {
        ...
        $sql = '
            ...
            inner join order_book
            where books.id=order_book.book_id_id
            AND order_book.user_email_id= :id AND order_book.processing=false;
        ';
        $stmt = $conn->prepare($sql);
        $stmt->execute(['id' => $userId]);
